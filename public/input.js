...
export function handleKeyInput(key) {
    ...
    if (!appState.state.room.started || appState.state.room.gameOver)
        return;
    if (appState.state.you.status !== "playing")
        return;
    const length = appState.state.room.currentLength;
    if (key === "ENTER") {
        const fixed = getFixedLetters();
        const composed = appState.currentGuess.map((ch, idx) => ch || (appState.overrideMask[idx] ? "" : fixed[idx]) || "");
...
export function setupKeyboardEvents() {
    document.addEventListener("keydown", (event) => {
        ...
        if (key === "BACKSPACE") {
            ...
            return handleKeyInput("BACK");
        }
        if (key === "ENTER") {
            event.preventDefault();
            return handleKeyInput("ENTER");
        }
        if (/^[A-Z]$/.test(key)) {
            event.preventDefault();

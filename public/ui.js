...
function getFocusedPlayer() {
    if (!appState.state?.room || !appState.state?.you)
        return null;
    ...
    if (!players.length)
        return null;
    const selected = players.find((player) => player.id === appState.selectedPlayerId) || players[0];
    return {
        ...selected,
...
export function buildGrid() {
    ...
    // Review and spectator boards are read-only, even when the selected player is still active.
    const isLiveSelfPlaying = Boolean(appState.state.you &&
        !appState.state.you.spectator &&
        appState.state.you.status === "playing" &&
        focusedPlayer.id === appState.state.you.id);
    const fixedLetters = isLiveSelfPlaying ? getFixedLetters() : Array(length).fill("");
    ...
    const containerWidth = Math.max(0, dom.grid.parentElement?.clientWidth || dom.grid.clientWidth || window.innerWidth - 40);
    ...
    for (let row = 0; row < appState.state.room.maxAttempts; row += 1) {
        ...
        for (let col = 0; col < length; col += 1) {
            ...
            else if (row === activeRow && isLiveSelfPlaying) {
...
export function buildPlayers() {
    ...
    if (!appState.state?.othersGrids)
        return;
    const selectorMode = Boolean(appState.state?.you?.spectator) || appState.state?.you?.status === "done";
...
export function renderWaitingRoom() {
    ...
    const modeLabel = appState.state.room.lengthMode === "fixed"
...
export function updateUI() {
    ...
    dom.podiumWatch.classList.toggle("hidden", !(appState.state.you?.status === "done" && !appState.state.room.gameOver));
    if (!appState.state.room.started) {
    ...
    dom.keyboard.classList.toggle("hidden", Boolean(appState.state.you?.spectator) || appState.state.you?.status === "done");
    dom.lobby.classList.toggle("hidden", appState.state.room.started || shouldShowPodium);
}

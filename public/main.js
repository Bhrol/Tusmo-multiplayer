...
dom.podiumWatch.addEventListener("click", () => {
    ...
    if (appState.state?.you?.status === "done") {
        const others = (appState.state.othersGrids || []).filter((player) => player.id !== appState.state.you.id);
        if (others.length) {
...
socket.on("room_state", (payload) => {
    ...
    if (canSelectPlayers) {
        const ownChoice = payload.you?.status === "done"
    ...
    if (payload.room.wordIndex !== appState.lastWordIndex ||

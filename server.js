...
function computeResult(guess, target) {
    const result = Array.from({ length: guess.length }, () => 0);
...
function pushHistory(player) {
    ...
    if (last && last.wordIndex === player.wordIndex)
        return;
    const attempts = player.attempts.map((attempt) => ({
        guess: attempt.guess,
...
io.on("connection", (socket) => {
    ...
    socket.on("submit_guess", (payload) => {
        ...
        const expectedLength = room.settings.mode === "timed" ? player.currentLength : room.currentLength;

 ...
 *
 * `you` contains letters for the recipient only. Other players are masked while
 * the recipient is still playing, then exposed for spectators and review mode.
 */
export function buildRoomState(room, socketId, maxAttempts) {
    ...
    const spectatorTimedWordIndex = roomPlayers.reduce((maxIndex, player) => Math.max(maxIndex, player.wordIndex ?? -1), -1);
    const viewWordIndex = room.settings.mode === "timed"
        ? you?.wordIndex ?? spectatorTimedWordIndex
        : room.wordIndex;
    const viewLength = room.settings.mode === "timed" ? you?.currentLength ?? room.currentLength : room.currentLength;
    const viewFirstLetter = room.settings.mode === "timed" ? you?.firstLetter ?? room.firstLetter : room.firstLetter;
    const othersGrids = roomPlayers
        ...
        grid: buildGridForPlayer(p, room.settings.mode === "timed" ? p.currentLength : room.currentLength, maxAttempts),
        status: p.status,

 ...
 */
function buildGridForPlayer(player: Player, length: number, maxAttempts: number): number[][] {
  const grid: number[][] = [];
 ...
 */
export function buildRoomState(room: Room, socketId: string, maxAttempts: number) {
  ...
  const spectatorTimedWordIndex = roomPlayers.reduce(

...
type SocketPayload = Record<string, unknown> | null | undefined;

/**
 * Compute Wordle-style result codes: 2=correct, 1=present, 0=absent.
 */
function computeResult(guess: string, target: string): TileResult[] {
  const result: TileResult[] = Array.from({ length: guess.length }, () => 0);
 ...
 */
function checkAdvance(room: Room): void {
...
function buildLengthSequence(settings: RoomSettings, initialLength: number): number[] {
...
function buildWordList(room: Room): string[] | null {
  const list: string[] = [];
...
function advancePlayerWord(room: Room, player: Player): void {
  pushHistory(player);
  player.wordIndex += 1;
...
function finalizePodium(room: Room): void {
  const now = Date.now();
...
function pushHistory(player: Player): void {
  ...
  const attempts: Attempt[] = player.attempts.map((attempt) => ({
    guess: attempt.guess,
...
function recordSyncHistory(room: Room): void {
  room.players.forEach((player) => {
    pushHistory(player);
  });
}
...
io.on("connection", (socket) => {
  ...
  socket.on("create_room", (payload: SocketPayload) => {
    removeFromRooms();
    const wordCount = Math.max(1, Math.min(20, Number(payload?.wordCount) || 1));
    const mode: GameMode = payload?.mode === "timed" ? "timed" : "sync";
    const language: Language = payload?.language === "en" ? "en" : "fr";
    const lengthMode: LengthMode = payload?.lengthMode === "range" ? "range" : "fixed";
  ...
  socket.on("join_room", (payload: SocketPayload) => {
    removeFromRooms();
  ...
  socket.on("set_name", (payload: SocketPayload) => {
    const code = normalizeCode(payload?.code);
    const room = rooms.get(code);
    if (!room) return;
  ...
  socket.on("start_game", (payload: SocketPayload) => {
    const code = normalizeCode(payload?.code);
    const room = rooms.get(code);
    ...
    const wordList = buildWordList(room);
    if (!wordList) {
      room.gameOver = true;
      socket.emit("error_msg", { text: "Could not build a word list for these settings." });
      emitRoomState(room);
      return;
    }
    room.wordList = wordList;
    console.log(`Room ${room.code} starting game with ${room.wordList.length} words.`);
  ...
  socket.on("restart_room", (payload: SocketPayload) => {
    const code = normalizeCode(payload?.code);
    const room = rooms.get(code);
  ...
  socket.on("submit_guess", (payload: SocketPayload) => {
    const code = normalizeCode(payload?.code);
    const room = rooms.get(code);
